...
print(f, "Brain Region\tTotal Area\tTotal Count\tAverage Count per Area");
...
//goes through every image and calls processFile 
function processFiles(dirImages, dirResults) {
	...
	totalResultsFile += "Brain Region\t\tArea\tCount\tCount per Area\n";
...
function makePolygonFromArray(arr, ratiow, ratioh, totalWidth) {
   ...
   //draws polygon with constant length around the specified brain region
...
function print_line(f, partNames, d, i, atlasNum) {
	//calculates count per area by dividing count by area
	countPerArea = 0;
	if (parseFloat(d[i*2]) > 0) {
		countPerArea = parseFloat(d[i*2+1]) / parseFloat(d[i*2]);
	}
 
    //adds result to the consolidated data file
    newLine = partNames[i] + "\t" + atlasNum + "\t" + d[i*2] + "\t" + d[i*2+1] + "\t" + countPerArea;
	...
	for (j = 0; j < brainRegionsArray.length; j++) {
		if (toLowerCase(partNames[i]) == toLowerCase(brainRegionsArray[j])) {
			totalBrainRegionCounts[j] += parseFloat(d[i*2+1]);
			totalBrainRegionSizes[j] += parseFloat(d[i*2]);
		}
	}
...
//counts cells based on 20-100 and returns an updated array with the new cell count appended
...
function processFile(imagePath, dirResults, atlasNum) {
	...
	//rest of the text file is the coordinates for the parts that are to be analyzed
	for (i = 2; i < lines.length; i ++) {
		partCoordinates = split(lines[i], ",");

		//parses all of the coordinates into integers
		for (j = 0; j < partCoordinates.length; j++) {
			partCoordinates[j] = parseInt(partCoordinates[j]);
		}

		//makes polygon and analyzes it
		selectWindow(name);
		makePolygonFromArray(partCoordinates, ratiow, ratioh, w);		
		data = analyze_pic(data);
	}

	//opens correctly named txt file for results based on left/right side

	f = File.open(dirResults + imageName + "_data.csv");
	
	print(f, "Brain Region\tArea\tCount\tCount per Area");
	...
	//rescales atlas image and overlays it over slide, then saves it in results folder
	overlay_image(imagesDir, name, imageName, atlasPath, dirResults, atlasNum);
}
