...
function processFile(imagePath, dirResult, atlasNum) {
	...
	for (i = 2; i < lines.length; i ++) {
		...
		//makes polygon and analyzes it. The data array is modified by the analyze_pic function to include data on the ROI drawn.
		selectWindow(name);
...
function analyze_pic(dataArray) {
    ...
    dataArray = append(dataArray, numberParticles);
    return dataArray;
}
...
function print_line(f, partNames, dataArray, i, atlasNum) {
    ...
    newLine = partNames[i] + "\t" + atlasNum + "\t" + dataArray[i*2] + "\t" + dataArray[i*2+1] + "\t" + countPerArea;
    ...
    print(f, partNames[i] + "\t" + dataArray[i*2] + "\t" + dataArray[i*2+1] + "\t" + countPerArea);
}
